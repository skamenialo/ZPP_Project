...
namespace ZPP_Project
{
    ...
    public class ApplicationUserManager : UserManager<ZppUser, int>
    {
        ...
        {
            var manager = new ApplicationUserManager(new UserStore<ZppUser, ZPPRole, int, ZPPUserLogin, ZPPUserRole, ZPPUserClaim>(context.Get<ZppIdentityContext>()))
            {
                PasswordHasher = new SHA512PasswordHasher()
            };
            // Configure validation logic for usernames
            manager.UserValidator = new UserValidator<ZppUser, int>(manager)
            {
            ...
            manager.RegisterTwoFactorProvider("Phone Code", new PhoneNumberTokenProvider<ZppUser, int>
            {
                MessageFormat = "Your security code is {0}"
            });
            manager.RegisterTwoFactorProvider("Email Code", new EmailTokenProvider<ZppUser, int>
            {
            ...
            if (dataProtectionProvider != null)
            {
                manager.UserTokenProvider = 
                    new DataProtectorTokenProvider<ZppUser, int>(dataProtectionProvider.Create("ASP.NET Identity"));
            }
            return manager;
        }
    ...
    public class ApplicationSignInManager : SignInManager<ZppUser, int>
    {
        ...
        public override Task<ClaimsIdentity> CreateUserIdentityAsync(ZppUser user)
        {
